...
function PhongBan() {
  ...
  const sendRequest = async (method = "GET", data = {}) => {
    if (method === "GET") {
      return await CRUDPhongban(method);
    }

    if (data) {
      return await CRUDPhongban(method, data);
    }
  };
  const store = useMemo(() => {
  ...
  return (
    ...
    >
      <Editing
        ...
        allowDeleting={true}
      >
        <Popup title="Phòng ban" showTitle={true} width={700} height={525} />
      </Editing>
      <Column
